...
|---|---|---|
| Un solo `<h1>` por página | ✅ | Confirmado en las 4 páginas |
| HTML semántico (`header`, `nav`, `main`, `section`, `footer`) | ✅ | Estructura consistente en las 4 páginas |
| Foco visible en enlaces/botones (`:focus-visible`) | ✅ | Definido globalmente en `estilos.css` |
| Todos los campos de formulario con `<label>` | ✅ | Verificado en `contacto.html`: 5/5 campos con `for`/`id` coincidentes |
| Enlace "Saltar al contenido" | ✅ | Presente en las 4 páginas |
| `aria-current="page"` en el link activo del menú | ✅ | Verificado en las 4 páginas |
| `aria-labelledby` apunta a un `id` existente | ✅ | Se corrigió un caso en `nosotros.html` donde apuntaba a un id inexistente |
| Contraste de color (WCAG AA, mínimo 4.5:1 texto normal) | ⚠️ → ✅ | Se detectó que el botón (`.boton`, verde claro `#00A44F`) tenía contraste insuficiente (3.27:1). Corregido usando `--color-verde-oscuro` (5.46:1) |
